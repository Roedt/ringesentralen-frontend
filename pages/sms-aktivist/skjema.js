...
function SMSSkjema ({ setSuccess }) {
  ...
  async function handleSubmit (event) {
    ...
    setLoading(true)
    const token = await getToken({ action: 'sms-aktivist' })
    const form = document.getElementById('sms-aktivist-skjema')
    const payload = generatePayload(form)
    payload.token = token
    await axios.post('/api/sms-aktivist', payload)
    setLoading(false)
    form.reset()
    setSuccess(true)
  }
  ...
  return (
    <form id='sms-aktivist-skjema' onSubmit={handleSubmit} className='grid grid-cols-1 gap-y-6'>
      <div>
