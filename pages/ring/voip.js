...
function VoIP ({ telefonnummer }) {
  ...
  const StatusLinje = ({ status }) => {
    ...
    return (
      <div className={`mt-2 flex items-center text-gray-900 font-medium text-lg ${statusFarger[status]} py-2`}>
  ...
  if (!telefonnummer) {
    const message = 'Brukeren har ikke telefonnummer. Da kan ikke vedkommende ringes.'
    return (
      <div className='mb-4 pb-4 border-b border-gray-200'>
        <Warning message={message} />
      </div>
