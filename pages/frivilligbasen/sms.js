...
import { erAdmin, is401, is403 } from '../../lib/utils'
import { Warning } from '../../components/ui/alerts'
import Spinner from '../../components/ui/spinner'

function SenderSms () {
  return (
    <div className='w-full py-3 mt-10 bg-gray-800 rounded-sm font-medium text-white uppercase focus:outline-none flex justify-center'>
      <span className='mr-4'>Sender sms, vennligst vent</span><Spinner />
    </div>
  )
}
...
function SMS ({ frivillige, filterKriterier, user }) {
  ...
  if ((filterKriterier && filterKriterier.length === 0) || (frivillige && frivillige.length === 0) || (user && !erAdmin(user.rolle))) return null
